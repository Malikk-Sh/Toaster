...
// Масштаб живучести/силы врага: изначально ×2.25, с прогрессом (зона×волна) до ×6
// (базовые 1.5→4 ещё ×1.5 по просьбе — враги живучее). Сверху множитель Новой Игры+.
function enemyScale(){
  const progress = clamp(((game.zone||0)*10 + Math.max(0,Spawner.wave)) / 40, 0, 1);
  return clamp(1.5 + progress*2.5, 1.5, 4) * 1.5 * diffMul();
}
